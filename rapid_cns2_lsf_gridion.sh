...
bsub -n 4 -R "rusage[mem=64G]" -q verylong -w "done($MEGALODON_JOB)" -J ${MC_JOB} -e ${LOG_DIR}/methylation_classification.err -o ${LOG_DIR}/methylation_classification.log Rscript scr/methylation_classification_nanodx.R --sample=${SAMPLE} --out_dir=${MC_DIR} --megalodon_dir=${MEGALODON_DIR} --probes_file=${PROBES} --training_data=${TRAINING_DATA} --threads=4 --array_file=${ARRAY_FILE}
