...
mkdir -p ${LIB_PATH}/deepvariant_target/
...
bsub -gpu num=1:j_exclusive=yes:mode=exclusive_process:gmem=24G -q gpu -J ${MEGALODON_JOB} -L /bin/bash bash scr/megalodon.sh --fast5_dir=${FAST5_DIR} --out_dir=${MEGALODON_DIR} --guppy_path=${GUPPY_PATH} --reference_file=${MMI_HG38}
...
bsub -n 32 -R "rusage[mem=64G]" -q long -e ${LOG_DIR}/align.err -o ${LOG_DIR}/align.log -J ${ALIGN_JOB} bash scr/alignment_dna.sh --fastq_dir=${FASTQ_DIR} --out_dir=${LIB_PATH}/bam --sample=${SAMPLE} --threads=32 --reference=${MMI_HG19} --targets=${TARGETS}
...
bsub -n 32 -R "rusage[mem=64G]" -q long -w "done($ALIGN_JOB)" -J ${DEEPVARIANT_JOB} bash scr/deepvariant.sh  --bam_file=${LIB_PATH}/bam/${SAMPLE}_cat.fastq.minimap2.coordsorted.ontarget.bam --out_dir=${LIB_PATH}/deepvariant_target --reference=${REFERENCE_HG19} --sample=${SAMPLE} --threads=32 --path=${DEEPVARIANT_PATH}
DEEPVARIANT_ANNO=${SAMPLE}_deepvariant_annovar
bsub -n 2 -R "rusage[mem=20G]" -q long -w "done($DEEPVARIANT_JOB)" -J ${DEEPVARIANT_ANNO} -e ${LOG_DIR}/deepvariant_annovar.err -o ${LOG_DIR}/deepvariant_annovar.log bash scr/deepvariant_annovar.sh --bam_file=${LIB_PATH}/bam/${SAMPLE}_cat.fastq.minimap2.coordsorted.ontarget.bam --out_dir=${LIB_PATH}/deepvariant_target --annovar_path=${ANNOVAR_PATH} --annovar_db=${ANNOVAR_DB} --panel_bed=${PANEL} --reference=${REFERENCE_HG19} --sample=${SAMPLE}

#Variant and SV calling
bsub -n 2 -R "rusage[mem=32G]" -q long -w "done($ALIGN_JOB)" -J ${SAMPLE}_svim -e ${LOG_DIR}/svim.err -o ${LOG_DIR}/svim.log bash scr/svim.sh --bam_file=${LIB_PATH}/bam/${SAMPLE}_cat.fastq.minimap2.coordsorted.ontarget.bam --out_dir=${LIB_PATH}/svim_target --reference=${REFERENCE_HG19} --sample=${SAMPLE}

LONGSHOT_JOB=${SAMPLE}_longshot
bsub -n 2 -R "rusage[mem=32G]" -q long -w "done($ALIGN_JOB)" -J ${LONGSHOT_JOB} -e ${LOG_DIR}/longshot.err  -o ${LOG_DIR}/longshot.log bash scr/longshot.sh --bam_file=${LIB_PATH}/bam/${SAMPLE}_cat.fastq.minimap2.coordsorted.ontarget.bam --out_dir=${LIB_PATH}/longshot --annovar_path=${ANNOVAR_PATH} --annovar_db=${ANNOVAR_DB} --panel_bed=${PANEL} --reference=${REFERENCE_HG19} --sample=${SAMPLE}
...
bsub -R "rusage[mem=24G]" -q long -w "done($MEGALODON_JOB)" -J ${MGMT_JOB} bash scr/mgmt_sites.sh --megalodon_dir=${MEGALODON_DIR} --mgmt_bed=${MGMT_BED}
...
bsub -n 4 -R "rusage[mem=64G]" -q long -w "done($MEGALODON_JOB)" -J ${MC_JOB} -e ${LOG_DIR}/methylation_classification.err -o ${LOG_DIR}/methylation_classification.log Rscript scr/methylation_classification_nanodx.R --sample=${SAMPLE} --out_dir=${MC_DIR} --megalodon_dir=${MEGALODON_DIR} --probes_file=${PROBES} --training_data=${TRAINING_DATA} --threads=4 --array_file=${ARRAY_FILE}

#Filter SNPs
DEEPVARIANT_REPORT=${SAMPLE}_DV_REPORT
LONGSHOT_REPORT=${SAMPLE}_LONGSHOT_REPORT
bsub -n 2 -R "rusage[mem=20G]" -q long -w "done($DEEPVARIANT_ANNO)" -J ${DEEPVARIANT_REPORT} -e ${LOG_DIR}/deepvariant_report.err -o ${LOG_DIR}/deepvariant_report.log Rscript scr/filter_report.R --input=${LIB_PATH}/deepvariant_target/${SAMPLE}_deepvariant_panel.hg19_multianno.csv --output=${LIB_PATH}/deepvariant_target/${SAMPLE}_deepvariant_report.csv --sample=${SAMPLE} 
