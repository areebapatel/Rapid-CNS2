...
GUPPY_JOB=${SAMPLE}_guppy
bsub -gpu num=1:j_exclusive=yes:mode=exclusive_process:gmem=20G -q gpu -J ${GUPPY_JOB} bash scr/guppy_dna.sh --fast5_dir=${FAST5_DIR} --out_dir=${GUPPY_DIR} --guppy_path=${GUPPY_PATH}
...
bsub -n 8 -R "rusage[mem=64G]" -q verylong -w "done($GUPPY_JOB)"  -e ${LOG_DIR}/trim.err -o ${LOG_DIR}/trim.log -J ${TRIM_JOB} bash scr/trimming.sh --fastq_dir=${FASTQ_DIR} --out_dir=${LIB_PATH}/fastq --sample=${SAMPLE} --porechop_path=${PORECHOP_PATH} --threads=8 
...
bash scr/deepvariant.sh  --bam_file=${LIB_PATH}/bam/${SAMPLE}_cat.fastq.trimmed.minimap2.coordsorted.bam --out_dir=${LIB_PATH}/deepvariant --reference=${REFERENCE_HG19} --sample=${SAMPLE} --threads=16 --path=${DEEPVARIANT_PATH} --job=${ALIGN_JOB}
...
bsub -R "rusage[mem=24G]" -q verylong -w "done($MEGALODON_JOB)"  -J ${MGMT_JOB} bash scr/mgmt_sites.sh --megalodon_dir=${MEGALODON_DIR} --mgmt_bed=${MGMT_BED} --reference_file=${MMI_HG38} --guppy_path=${GUPPY_PATH}
...
bsub -n 4 -R "rusage[mem=64G]" -q verylong -J ${MC_JOB} -w "done($MEGALODON_JOB)" -e ${LOG_DIR}/methylation_classification.err -o ${LOG_DIR}/methylation_classification.log Rscript scr/methylation_classification_nanodx.R --sample=${SAMPLE} --out_dir=${MC_DIR} --megalodon_dir=${MEGALODON_DIR} --probes_file=${PROBES} --training_data=${TRAINING_DATA} --threads=4 --array_file=${ARRAY_FILE}
